...
void agregar_valores_grafo(TGrafo *grafo, int operaciones);
...
int main()
{
	...
	scanf("%d %d",&max_vertices, &operaciones);
	inicializar_grafo(&grafo, NO_DIRIGIDO);
/*	agregar_valores_grafo(&grafo,operaciones);*/
...
void agregar_arista(TGrafo *grafo, int v1, int v2)
{
	if(grafo->vertices[v1]==true && grafo->vertices[v2]==true){
...
void agregar_arista_i(TGrafo *grafo, int v1)
{
	for (int v2 = 0; v2 < MAX_VERTICES; v2++)
	{
		if (v1 != v2)
		{
